...
const methods: CallableFunction[] = [onRawDataAvgMin, onRawDataAvgHour, onRawDataAvgDay, onRawDataMedianMin,
    onRawDataMedianHour, onRawDataMedianDay];
let start;
let methodIndex = -1;
let currentListener;
...
function checkTestFinished(data) {
    if  (new Date(data.endDateString) >= new Date(toDate) && methodIndex < methods.length) {
        ...
        fs.writeFile(`./src/Test/testData/cached/${methods[methodIndex].name}.txt`, JSON.stringify(cumulRaw), (err) => {
            if (err) {
